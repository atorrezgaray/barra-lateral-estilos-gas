//variable donde guaradmos todos los estilos
var estilos_sheet = PropertiesService.getDocumentProperties();

function onOpen() {
  SpreadsheetApp.getUi().createMenu('Menu Personalizado')
    .addItem('Mostrar barra lateral', 'mostrarBarraLateral')
    .addToUi();
}

function mostrarBarraLateral(){
  //var barra = HtmlService.createHtmlOutputFromFile('BarraLateral').setTitle('Barra Lateral Aula'); //anterior
...
function aplicarEstilo(estilo){
  ...
  //ui.alert('Entra funcion aplicarEstilo1()');

  //borrar estilos de las celdas activas
  borrarEstilos();
...
function guardarEstilo(estilo){
  //var ui = SpreadsheetApp.getUi();
  //ui.alert('Entra guardarEstilo()');

  //borrar previamente los estilos
  eliminarEstilo(estilo);

  //obtener la celda activa y los bordes
  var celda = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet().getActiveCell();

  //guardar los bordes
  guardarBordes(celda, estilo);

  estilos_sheet.setProperty('colorLetra'+estilo, celda.getFontColorObject().asRgbColor().asHexString())
...
function borrarEstilos(){
  SpreadsheetApp.getActiveSpreadsheet().getActiveSheet().getActiveRange().clear({formatOnly:true}); //borrar solo el formato
}

function borrarTodo(){
  //var ui = SpreadsheetApp.getUi();
  //ui.alert('Entra funcion borrarTodo()');
  SpreadsheetApp.getActiveSpreadsheet().getActiveSheet().getActiveRange().clear(); //borrar todo  
}
